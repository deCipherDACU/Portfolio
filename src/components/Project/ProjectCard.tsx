    ...
    return (
        ...
        >
            ...
            <div className="aspect-video bg-black relative overflow-hidden">
                ...
                */}
                ...
                <div className="absolute inset-0 p-6 flex flex-col justify-center items-center opacity-0 group-hover:opacity-100 transition-opacity z-20">
                    <h3 className="text-2xl font-bold text-white uppercase tracking-tighter text-center mb-2">{project.title}</h3>
                </div>
            </div>
            ...
            <div className="p-4 bg-black border-t border-white/10 flex justify-between items-end">
                <div>
                    <h3 className="text-lg font-bold text-white uppercase tracking-tight group-hover:text-primary transition-colors">{project.title}</h3>
                    <div className="flex gap-2 mt-2">
                        {[...project.typeTags, ...project.softwareTags].slice(0, 2).map(tag => (
                            <span key={tag} className="text-[9px] font-bold uppercase tracking-wider text-black bg-white/80 px-1.5 py-0.5 rounded-sm">
                                {tag}
                            </span>
                        ))}
                    </div>
                </div>
                <div className="text-[10px] font-mono text-white/40 group-hover:text-primary">
                    {project.year}
                </div>
            </div>
        </div>
